...
class FromTest : BaseUnitTest() {
    ...
    @Test
    fun validateSimpleFrom() = writableDatabaseForTable<SimpleModel> {
        assertEquals("SELECT * FROM `SimpleModel`", (select from SimpleModel::class).query.trim())
    }

    @Test
    fun validateProjectionFrom() = writableDatabaseForTable<SimpleModel> {
        assertEquals("SELECT `name` FROM `SimpleModel`", (select(name) from SimpleModel::class).query.trim())
    }

    @Test
    fun validateMultipleProjection() = writableDatabaseForTable<SimpleModel> {
    ...
    @Test
    fun validateAlias() = writableDatabaseForTable<SimpleModel> {
        assertEquals("SELECT * FROM `SimpleModel` AS `Simple`", (select from SimpleModel::class `as` "Simple").query.trim())
    }

    @Test
    fun validateJoins() = writableDatabaseForTable<SimpleModel> {
