...
class TriggerTest : BaseUnitTest() {
    ...
    fun validateBasicTrigger() {
        assertEquals("CREATE TRIGGER IF NOT EXISTS `MyTrigger` AFTER INSERT ON `SimpleModel` " +
                ...
                "\nEND",
                (createTrigger("MyTrigger").after() insertOn SimpleModel::class)
                        .begin(
                                insert(TwoColumnModel::class)
                                        .columnValues(name to NameAlias.ofTable("new", "name"))
                        )
        )
    }
    ...
    fun validateUpdateTriggerMultiline() {
        assertEquals("CREATE TRIGGER IF NOT EXISTS `MyTrigger` BEFORE UPDATE ON `SimpleModel` " +
                "\nBEGIN" +
                "\nINSERT INTO `TwoColumnModel`(`name`) VALUES(`new`.`name`);" +
                "\nINSERT INTO `TwoColumnModel`(`id`) VALUES(CAST(`new`.`name` AS INTEGER));" +
                "\nEND",
                (createTrigger("MyTrigger").before() updateOn SimpleModel::class)
                        .begin(insert(TwoColumnModel::class)
                                .columnValues(name to NameAlias.ofTable("new", "name")))
                        .and(insert(TwoColumnModel::class)
