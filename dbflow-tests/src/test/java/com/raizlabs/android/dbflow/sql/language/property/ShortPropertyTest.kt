...
class ShortPropertyTest : BaseUnitTest() {

    @Test
    fun testOperators() {
        val prop = ShortProperty(SimpleModel::class.java, "Prop")
        ...
        assertEquals("`Prop` LIKE '5'", prop.like(5).query.trim())
        assertEquals("`Prop` NOT LIKE '5'", prop.notLike(5).query.trim())
        assertEquals("`Prop` GLOB '5'", prop.glob(5).query.trim())
        ...
        assertEquals("`Prop` BETWEEN 5 AND 6", prop.between(5).and(6.0).query.trim())
    ...
    @Test
    fun testAlias() {
        val prop = ShortProperty(SimpleModel::class.java, "Prop", "Alias")
        assertEquals("`Prop` AS `Alias`", prop.toString().trim())

        val prop2 = ShortProperty(SimpleModel::class.java,
