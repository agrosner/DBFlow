...
import com.raizlabs.android.dbflow.structure.ModelView;
...
public class FlowSQLiteOpenHelper extends SQLiteOpenHelper {
    ...
    private void executeMigrations(final SQLiteDatabase db, final int oldVersion, final int newVersion) {
        try {
            final List<String> files = Arrays.asList(AppContext.getInstance().getAssets().list(MIGRATION_PATH));
    ...
    private void executeSqlScript(SQLiteDatabase db, String file) {
        try {
            final InputStream input = AppContext.getInstance().getAssets().open(MIGRATION_PATH + "/" + file);
