...
object ProviderMethodUtils {
     ...
     */
    internal fun getSegmentsPreparation(uriDefinition: ContentUriDefinition): CodeBlock {
        if (uriDefinition.segments.size == 0) {
            return CodeBlock.builder().build()
        } else {
            return CodeBlock.builder().addStatement("\$T<\$T> segments = uri.getPathSegments()", List::class.java, String::class.java).build()
        }
    }
     ...
     */
    internal fun getSelectionAndSelectionArgs(uriDefinition: ContentUriDefinition): CodeBlock {
        val segments = uriDefinition.segments
        if (segments.size == 0) {
            ...
            val selectionArgsBuilder = CodeBlock.builder().add("\$T.appendSelectionArgs(selectionArgs, new \$T[] {", ClassNames.DATABASE_UTILS, String::class.java)
            var isFirst = true
